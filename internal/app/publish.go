...
type PublishRequest struct {
	...
	// Binding is the run binding of the execution that produced the checkpoint.
	//
	// It is a semantic.RunBinding rather than a bare ExecutionID on purpose. A
	// CheckpointArtifact deliberately does not carry an execution identity --
	// executor identity affects only ExecutionID, so the same checkpoint can be
	// produced by different executions of one semantic run -- which means a bare
	// identifier here could name an execution unrelated to the checkpoint, and the
	// publication record's whole value is that it can be audited. Only
	// semantic.BindRun produces a binding, and a binding commits to both
	// SemanticRunID and ExecutionID together, so the pairing is authenticated
...
func Publish(
	...
	current, published, err := stores.Publications.CurrentPublication(
		ctx, request.TenantID, request.CustomerID, request.Target)
	if err != nil {
		return PublicationOutcome{}, fmt.Errorf("app: current publication could not be read: %w", err)
	}

	next := publicationFor(request, policy, current.Version+1)

	// Re-publishing exactly what is already there appends nothing.
	//
	// This is the normal case rather than an edge case: execution delivery is
	// at-least-once by design, since a lease can expire and a second attempt
	// reproduces byte-identical artifacts. Without this, an expired lease would
	// leave a target's history showing the same checkpoint published twice, and
	// nothing later could tell that from two real decisions.
	...
	if err := stores.Publications.Publish(ctx, next); err != nil {
		// A conflict is not a gate refusal. The pointer moved between the read and
		// the write, so the caller's view is stale and the fix is to read again --
		// which is exactly the silent overwrite §14.1 requires to fail.
...
func publicationFor(
	...
	return ports.Publication{
		...
		SemanticRunID:        request.Binding.SemanticRunID(),
...
func validatePublishRequest(request PublishRequest) error {
	...
	// Kernel values are authenticated by their types, but every exported struct
	// type still has a constructible zero value, so absence is checked explicitly.
	checkpoint := request.Candidate.Checkpoint
	if checkpoint.ID() == "" || request.Candidate.Assessment.ID() == "" ||
		request.Binding.ExecutionID() == "" {
		return InvalidInputError{Code: InputPublishRequestIncomplete}
	}
	if request.Binding.SemanticRunID() != checkpoint.SemanticRunID() {
		// The identities are deliberately not named in the message. This package's
		// errors carry a closed code and fixed safe text and never interpolate an
		// identifier, so an error surfaced to a caller cannot become a channel for
		// content. The mismatch is diagnosable from the request the caller already
		// holds.
		return InvalidInputError{Code: InputPublishBindingMismatch}
	}
	return nil
}
