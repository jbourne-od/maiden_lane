...
func TestARefusalNamesEveryClauseAndItsReason(t *testing.T) {
	...
	// Three refusals: comparison corpus, no metric regression, and certified backend
	// are all implemented in this build and refuse for want of evidence on this candidate.
	...
	for _, clause := range []promotion.Clause{
		...
		promotion.ClauseNoMetricRegression,
		promotion.ClauseCertifiedBackend,
	} {
		if got := reasons[clause]; got != promotion.InformationAbsent {
			t.Fatalf("clause %v reason = %v, want InformationAbsent: it is "+
				"implemented, and this candidate simply carries no evidence for it", clause, got)
		}
	}
}
