...
step_1_create_dynamic_drivers_selected AS (
    SELECT s.*, 
        ...
        ROW_NUMBER() OVER (PARTITION BY (s."home_terminal_zip") ORDER BY decode(SUBSTRING(s."id" FROM 8), 'hex')) AS _ml_row_num,
        COUNT(*) OVER (PARTITION BY (s."home_terminal_zip")) AS _ml_progenitor_count,
        STRING_AGG(SUBSTRING(s."id" FROM 8), '') OVER (
            PARTITION BY (s."home_terminal_zip")
...
step_1_create_dynamic_drivers_new_entities AS (
    SELECT 
        ...
        || decode(LPAD(TO_HEX(OCTET_LENGTH(convert_to('dynamic_driver', 'UTF8'))), 16, '0'), 'hex')
        || convert_to('dynamic_driver', 'UTF8')
        || decode(LPAD(TO_HEX(OCTET_LENGTH(convert_to('create_dynamic_drivers', 'UTF8'))), 16, '0'), 'hex')
